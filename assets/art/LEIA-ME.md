...
Imagens que as telas usam. Vieram do app web original (`Aplicativos/lembreiAI/public/assets`) e foram copiadas **idênticas** (SHA-256 conferido), exceto as três camadas de curvas de nível, que `scripts/arte/gerar-topo.sh` gera com o Chrome a partir do SVG, com o mesmo recorte e a mesma máscara do CSS do app web.
...
| `horizonte-contas.webp` | 112.534 | O horizonte no pé das telas de conta (`02`): as curvas viradas e esticadas, só a faixa em que há traço (1702 × 880) | gerado |
...
- **As camadas geradas têm 2 px por `du`** (1702 px de largura para a coluna de 851 du), o bastante para 3× em 430 dp. Rode `scripts/arte/gerar-topo.sh` de novo se o SVG ou os modelos em `scripts/arte/*.html` mudarem (ou `scripts/arte/gerar-topo.sh horizonte-contas`, para regravar só uma). `src/design/__tests__/arte.test.ts` confere que o horizonte tem 1702 × 880, a proporção que o token `size.auth.horizonteAltura` espera.

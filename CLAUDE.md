...
- **Depois de entrar (decisão do João, 21/09):** o app abre no formulário de novo lembrete, não em Início. É o `initialRouteName="novo"` das abas em `app/(app)/_layout.tsx` (o `Stack.Protected` da raiz cai na rota inicial do grupo), nunca uma navegação à mão. Só vale para o ato de entrar: abrir o app já entrado por um endereço segue o endereço. `src/__tests__/navegacao.test.tsx` cobre, com uma réplica do guard amarrada ao `app/_layout.tsx`.

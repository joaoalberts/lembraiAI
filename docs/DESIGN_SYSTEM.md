...
| `../DESIGN_SYSTEM.md` e `../ref/` (app web, pasta acima) | A medição por pixel do app web e as referências antigas (`2.png`, `4.png` e `5.png`), superadas pelas capturas. |
| `../src/styles/tokens.css` | Os valores que o app web de fato usa (nomes de cor iguais: `forest`, `mint`, `orange`, `cream`, `ink`). |
...
| `colors.onboarding.bg` | `#12301F` | `palette.onboardingBg` | Fundo do Onboarding |
...
- **Etiqueta** (`src/components/Tag.tsx`): pílula na cor da categoria (`colors.category.*.tag`), ícone de pino em `.fg` e o texto "Por horário" ou "Por local" em `.tagInk` (o `fg` escurecido até 4,5:1; o azul e o rosa da imagem dão 2,7:1 e 2,4:1), em `textStyles.pico` (9 dp: o original tem 8,5 e o piso é 9).
...
- **Texto no tamanho do original** (decisão do João, 21/09): título de 12 (`textStyles.micro` em serifa; `.title` 23,3 du), data, lugar e "Você está aqui" de 9 (`textStyles.pico`; `.meta` 18,6 e `.place` 18 du) e a hora de 11 (`textStyles.mini`; `.hour` 22,5 du). Antes eram 14 e 12 (o piso) e o cartão saía mais alto que na imagem; agora tem a mesma altura (cerca de 78 a 430 px). `size.card.minHeight` é só o mínimo: o cartão cresce com o texto e com a fonte grande do sistema.
...
`src/components/Onboarding.tsx`, numa única rota, `app/auth/bem-vindo.tsx` (primeira tela de quem ainda não entrou; os dois botões levam ao login, sem barra de abas, com a base respeitando a área segura). Depois de entrar o app não a mostra mais: não há aba nem rota para ela em `app/(app)/`. Fundo liso `colors.onboarding.bg` — **sem foto** (decisão do João, 22/09: a foto desfocada `bg-onboarding.jpg` do app web saiu; ver 18) — e por cima um único fluxo vertical:
...
- **Desvios das imagens:** o nome da marca em serifa 700 nas listas (a captura usa um peso a menos), sem a sombra do título e sem o halo escuro em volta dos textos pequenos (a arte tem; o CSS não).
...
- **Local ligado:** a busca de endereço (`PlaceSearch`, sobre o funil de `src/lib/geocodificar.ts`: Photon e Nominatim em paralelo, com CEP resolvido pelo ViaCEP e reforço do Nominatim por rua/número — todos gratuitos e sem chave; só com 3 letras ou mais, 650 ms depois da última, pedido anterior cancelado; até cinco sugestões, do Brasil inteiro, com rua, número, bairro, cidade, estado e CEP quando o serviço souber; sem resultado diz "Nenhum endereço encontrado. Confira o nome da rua ou toque no mapa."; falha diz "Não foi possível buscar agora."; um resultado cujo número de porta o mapa não tem volta como a rua, com o aviso "Posição aproximada: confira o pino no mapa" na sugestão e, escolhido, no lugar da dica do raio), o mapa (`MapaDeEscolha`, Leaflet + OpenStreetMap, `size.form.map` de altura; toque ou arrasto do pino põe o ponto e o nome do lugar vem do serviço 700 ms depois; o círculo mostra o raio), o raio (50 a 550 m de 10 em 10, padrão 150) e a dica "Você será avisado ao entrar no raio selecionado.". No mapa flutuam três botões: centralizar no pino, zoom e "Usar minha localização" (`colors.map.control`, `shadow.float`). A roda do mouse só dá zoom no mapa depois de um clique, senão rolar a página com o cursor em cima dele o faria dar zoom sozinho. **Toque:** em tela de toque (`@media (pointer: coarse)`) os botões do mapa chegam a 44 (centralizar, zoom da aba Mapa e a pílula "Usar minha localização", de 24 para 44 de altura) e o zoom do mapa pequeno do formulário some (44 + 88 de botões não cabem nos 138 de altura, e a pinça dá o zoom); no computador ficam as medidas das imagens. **Teclado:** a lista de sugestões nasce debaixo do campo de busca, justamente onde o teclado fica; com a busca em foco e o teclado aberto (`useAlturaDoTeclado`) o formulário rola até o cartão do Local ficar no alto da área visível, abaixo da barra de status (conferido no Android, onde as sugestões apareciam atrás do teclado). **Toque:** a lista passa por cima do mapa e, no Android, o toque numa sugestão chegava também à WebView do mapa embaixo, que trocava o ponto escolhido (e o pino saía da vista); enquanto a lista está aberta o mapa fica sem toque (`pointerEvents: 'none'` no estilo do contêiner dele).
...
A tela da imagem `09` (rota `sucesso?id=…`, `app/(app)/sucesso.tsx`): aparece depois de criar um lembrete e lê o lembrete pelo `id`. Fica sobre a foto de folhagem `assets/art/bg-success.jpg` (`contentFit="cover"`, presa ao topo, parada; a coluna rola por cima) num canvas de `size.sucesso.canvas` de altura (1848 du): em tela mais baixa rola, e o link "Criar outro lembrete" só aparece rolando, como nas capturas. **Não é uma aba**: a barra de abas fica escondida (`tabBarStyle: { display: 'none' }`), então a tela sempre tem saída (Fechar, "Ver todos os lembretes" e, sem lembrete, um botão para a lista). Só monta com a aba em foco: as abas ficam montadas e o herói tem um pulso infinito que não deve rodar escondido; chegar de novo repete a animação. Em aparelho com barra de status alta (entalhe, ilha) tudo desce o que a barra mais o respiro do alvo de toque (`respiroDoToque`) passar da distância do botão de fechar (`size.sucesso.fecharTop`). As posições vêm dos vãos `size.sucesso.*` (diferença entre as posições medidas) e não de coordenadas soltas.
...
- **Lembretes por local:** o interruptor "Monitorar lembretes por local" liga e desliga a vigia. Aviso vermelho se a localização deu erro. Linhas: Permissão de localização ("permitida" ou "não permitida"), Lembretes monitorados, Dentro do raio agora e Mais próximo ("título · N m") só monitorando, Última posição ("há 30s", "há 2 min" ou "nunca", atualizada a cada `motion.duration.relogio`; "—" desligado) e Precisão do sinal ("± N m"; "—" desligado). Aviso informativo "Último aviso: título (há 5 min)." quando houve chegada. Abaixo, o estado do **aviso com o app fechado** (geofence do sistema, `useSegundoPlano`): ativo (duas notas, avisando a demora de alguns minutos e o raio mínimo de 100 m), sem a permissão de localização "o tempo todo" (nota mais o botão "Permitir localização o tempo todo", ou só a nota se a permissão já foi negada de vez), indisponível (nota própria para a web e para o Expo Go) ou erro ao ativar.
- **Notificações:** fora da web, "Estado: permitida." ou "não permitida." (com o aviso vermelho de como liberar caso não esteja) e "Avisos por horário agendados". Na web, o subtítulo já diz "Avisos na tela enquanto o app está aberto." com o aviso informativo "No navegador os avisos só chegam com o app aberto: com a aba fechada nada é entregue.", o estado da permissão do navegador (permitida, bloqueada, indisponível — com a nota de que no iPhone só o app instalado na tela de início mostra avisos do sistema — ou o botão "Permitir avisos do navegador" enquanto pendente) e "Avisos por horário nas próximas 24 h" no lugar de "agendados" (a agenda do navegador não é a do sistema operacional).
...
A base das quatro telas é o `AuthLayout` (`src/components/AuthLayout.tsx`, imagem `02`), de cima para baixo: o fundo verde em degradê (`gradients.contas`) com as curvas de nível no alto (`assets/art/topo-contas.webp`, numa caixa da altura da arte, `size.auth.curvasAltura`: numa caixa mais baixa elas saem achatadas), ambos decorativos e sem receber toque; o botão redondo de voltar (`GlassButton` `tamanho="voltar"`, só quando a tela diz para onde voltar); a marca (tile menta com o símbolo e "LembreiAi" em serifa, `fontSize.contaMarca`); o **cartão creme** (`colors.bg.card`, raio `size.auth.cardRadius`, `shadow.cartaoDeConta`) com o título em serifa (`fontSize.contaTitulo`, é o cabeçalho para o leitor de tela), o subtítulo em `colors.text.secondary` e o formulário; e o rodapé. Com pergunta, o rodapé é a **barra de vidro** (`colors.conta.barra`, contorno `colors.conta.barraAnel`) com a pergunta ("Ainda não tem conta?") e a **pastilha** de ação (`colors.conta.pilula`, texto `colors.conta.pilulaTexto`); a pergunta ocupa o espaço que sobra ao lado da pastilha (`flex: 1`), e não só o do próprio texto: no Android, com a caixa do tamanho justo, "Já tem conta?" saía "Já tem" (a última palavra quebrava e sumia; causa exata não isolada); sem pergunta, só o link de voltar (`colors.conta.link`). Por último, a nota "Por onde você passa fica no seu aparelho." com o cadeado (`colors.conta.nota`). Com barra de status (celular), tudo desce o que a barra mais o respiro do alvo de toque (`respiroDoToque`, 4) passar da distância do botão de voltar: o sistema fica com o toque dentro da barra, e sem o respiro a folga de cima do botão se perderia. Na web nada se mexe. As medidas são `size.auth.*`.

**O horizonte** (`.horizonte` do `AuthShell` do app web): as mesmas curvas de nível do topo, viradas de cabeça para baixo e esticadas até a altura de 1000 du, com a máscara `linear-gradient(transparent 0 28%, #000 44%, rgba(0,0,0,.5) 58%, transparent 72%)`. O traço só existe entre 28% e 72% dessa altura, então a arte (`assets/art/horizonte-contas.webp`, 1702 × 880, gerada por `scripts/arte/gerar-topo.sh horizonte-contas`) guarda só essa faixa: `size.auth.horizonteAltura` de altura, com a base `size.auth.horizonteBase` acima do fim do conteúdo. Vai de ponta a ponta e é o **primeiro filho da rolagem**: fica atrás de tudo (o cartão, a barra de vidro e a nota desenham por cima) e não recebe toque. Acompanha o fim do conteúdo: nas telas altas (cadastro) aparece ao rolar. No app web o horizonte desenha por cima do cartão (acidente de empilhamento: o `.cartao` não é posicionado e o `.horizonte` é), o que risca de claro o botão laranja do cadastro; o CSS do original diz que ele "fica atrás de tudo", e é o que o app faz. Em Entrar a diferença é quase invisível (a máscara é fraca ali). Conferido no navegador a 430 px, lado a lado com a imagem `02`: as mesmas curvas nos mesmos lugares, com o mesmo esmaecimento sobre a barra de vidro.
...
- Na web não há efeito (o navegador desenha a barra do sistema).
- **Conferido no Android** (emulador, API 36, tela cheia, APK de teste): a abertura (foto escura) e as telas de conta saem com relógio, sinal e bateria em claro; a política de privacidade (topo claro) sai em escuro; voltando dela à abertura o claro volta sozinho. As telas com sessão (lista e configurações, formulário, mapa e sucesso) só foram conferidas por teste com a navegação de verdade (`src/__tests__/barra-de-status-navegacao.test.tsx`, que troca de aba com as abas montadas), não no aparelho: exigem entrar no app. **iOS não verificado** (sem Xcode neste Mac): o padrão foi escrito pela documentação do `expo-status-bar` (SDK 57).
...
- **Abertura (splash):** `splash-icon.png` (`imageWidth: 160`) sobre `colors.brand.tile`. **Notificação do Android:** `notification-icon.png` (96 por 96) tingido com `colors.action.primary`. **Web:** `public/favicon.svg`, `public/icons/*` (PWA 192, 512 e maskable) e o manifesto com o fundo `colors.bg.page` e a faixa do topo (`theme_color`, igual à metatag `theme-color` de `app/+html.tsx`) em `colors.bg.stage`.
...
| `size.auth.curvasAltura` | `263` | Contas: altura da camada de curvas de nível do topo (a da arte, 520 du; mais baixa, achataria as curvas) |
| `size.auth.horizonteAltura` | `222` | Contas: altura da faixa do horizonte (as curvas de nível espelhadas no pé da tela) |
| `size.auth.horizonteBase` | `141` | Contas: distância do fim do conteúdo até a base da faixa do horizonte |
...
- **Alvos de toque de no mínimo 44** (`size.touch`), no celular e na web (o app web será usado no celular: decisão do João, 21/09). **Todo controle passa por `Toque`** (`src/components/Toque.tsx`), que mede o próprio tamanho e completa o alvo até 44 **sem mexer no visual**: no iOS e no Android com o `hitSlop` do sistema e, na web, onde o react-native-web 0.21 não implementa `hitSlop`, com uma camada transparente por dentro do botão (o clique nela sobe até o botão; a largura da borda entra na conta). Nunca importe o `Pressable` do React Native nem escreva `hitSlop` à mão: `src/__tests__/alvos-de-toque.test.ts` barra. Onde dois controles ficam colados a camada do de baixo na árvore ganha no encontro. O controle deslizante (`Slider`) tem a área de 44 de altura com margem negativa que devolve o espaço do desenho. Os botões do mapa são DOM do Leaflet e não passam pelo `Toque`: `mapa-css.ts` os leva a 44 em tela de toque (`@media (pointer: coarse)`, que vale no celular e na WebView do app). Exceção: os créditos do OpenStreetMap e do Leaflet no canto do mapa, que são texto corrido. **Conferido no Android** (emulador, toques de verdade): a folga vale por inteiro onde há espaço (o link "Esqueci minha senha", de 16 de altura, aceita toque até 13 acima e abaixo do desenho; o "Voltar" das contas, de 36, até 4 à esquerda, à direita e embaixo). **Nenhum toque atravessa a barra de status** (medido: o sistema fica com todo toque dentro dela), então um botão encostado nela perde a folga de cima e o alvo fica curto (o "Voltar" das contas ficava com 44 × 40). **Decisão do João (21/09): alvo completo.** Quem ancora um botão logo abaixo da barra soma o respiro do alvo à área segura (`respiroDoToque(lado)`, em `Toque.tsx`: 4 no "Voltar" das contas, 3 no "Fechar" do sucesso); a tela desce essa diferença em relação à imagem, e sem barra (a web) nada se mexe. **Conferido no emulador Android depois da mudança:** o "Voltar" desceu 4 e o toque vale por inteiro nos quatro lados (em cima aceita até 10 px de fora, o que antes falhava já em 1 px). iOS não conferido.
...
- **Foco visível** no teclado (web) em todo controle: o mesmo anel sólido de `borderWidth.focus` em `colors.border.focus`, afastado `space.hair` (`src/design/foco.ts`), em botão, chip, opção do segmentado, lixeira do cartão e fechar da folha. **Só o foco de teclado desenha o anel** (`focoDeTeclado`, `src/design/foco.ts`, aplicado pelo `Toque`): na web um clique ou toque também dá foco ao botão e o react-native-web o avisa igual, mas o navegador diz que esse não é `:focus-visible`; sem essa conferência o anel ficava grudado na aba clicada. O campo de texto desenha o foco só pela borda e pelo halo `shadow.focus`, sem o contorno do navegador.
...
| 21/09/2026 | **Etiqueta do cartão com texto escurecido** (`colors.category.*.tagInk`); a lista, o cabeçalho verde, a marca, o botão de vidro e a busca entram como no original, com o texto no piso de 12 (revogado em 21/09: hoje no tamanho do original) | Na imagem o texto da etiqueta azul dá 2,7:1 e o rosa 2,4:1 sobre o fundo da etiqueta; o `fg` escurecido só até 4,5:1 mantém a cor e cumpre o teste. O original usa texto de 9 a 10 dp nas etiquetas, nos chips e nas datas: subiu para 12 e os cartões ficavam mais altos que na imagem (revogado em 21/09) |
...
| 21/09/2026 | **Horizonte espelhado no rodapé das telas de conta** (`assets/art/horizonte-contas.webp`, `size.auth.horizonteAltura` e `size.auth.horizonteBase`) | A imagem `02` mostra as curvas de nível também embaixo, espelhadas e esmaecendo para o verde (o `.horizonte` do `AuthShell`); faltava porque a arte não estava pré-renderizada. Gerada com a mesma receita e o mesmo Chrome sem janela das outras duas camadas de curvas, recortada na faixa em que há traço (28% a 72%: menos memória no celular) |

| 21/09/2026 | **As curvas de nível do topo das telas de conta com a altura da arte** (`size.auth.curvasAltura`, 520 du, no lugar de `size.header.height`, 345 du) | Achado ao comparar Entrar com a imagem `02` a 430 px: a arte `topo-contas.webp` tem 520 du de altura (o `.fundo` do original), mas era encaixada na caixa do cabeçalho da lista com `contentFit="fill"`, e as curvas saíam achatadas a ~66% (o anel grande da direita ficava baixo e largo). Um teste confere as dimensões do arquivo contra o token |

| 21/09/2026 | **O horizonte das telas de conta fica atrás de tudo, o cartão inclusive** (primeiro filho da rolagem, e não dentro do rodapé) | No app web ele desenha por cima do cartão (o `.cartao` não é posicionado e o `.horizonte` é), o que risca de claro o botão laranja do cadastro, cujo cartão é mais alto (visto no navegador a 430 px); na imagem `02` o efeito no botão "Entrar" é quase invisível. O comentário do próprio CSS do original diz "atrás de tudo": o app segue a intenção, não o acidente |
...
| 21/09/2026 | **A lista no tamanho do original:** rótulo da aba, chip e etiqueta de 9 dp, data e lugar do cartão de 9, hora de 11, título do cartão de 12 (era 14), frase da marca de 9 nas listas e 11 na abertura, data da seção de 11 | Decisão do João (piso de 9). Conferido a 430 px ao lado da imagem `07`: o cartão volta à altura da imagem (cerca de 78) e as abas e os chips saem do mesmo tamanho |
| 21/09/2026 | **Sucesso, contas, config, formulário, folhas e onboarding também no tamanho do original** (seis commits, 1095 testes passam): sucesso já estava correto (60,5 du = ~30 dp para título, 29 du = ~15 dp para subtítulo). Contas/config: rótulos e pergunta 22-23 du (11 dp, mini); folhas: subtítulo 23 du (11 dp); formulário: rótulo 22-23 du, dica 18-20 du (pico); onboarding: balão/benefício 21 du (mini), privacidade 19,5 du (nano). Tudo reduzido de 12 dp (micro) conforme o CSS do app web. Todos os testes passam sem nenhuma falha | Tarefa `textos-do-tamanho-do-original` concluída em seis commits: lista (f876681), sucesso (c83cb3d), contas (7fb0ef8), config (2b7370e), formulário (c7a9b42), folhas (391cb1c), onboarding (5271daa) |
| 21/09/2026 | **Aviso por chegada num raio passa a existir com o app fechado** (geofence do sistema: `expo-location` + tarefa headless do `expo-task-manager`, registrada em `index.js` antes de `expo-router/entry`); Configurações ganha o cartão "Lembretes por local" com o estado do aviso em segundo plano (ativo, sem a permissão "o tempo todo", indisponível ou erro) e o cartão "Até onde vai o monitoramento" muda de texto quando ele está ativo | Pedido do João: "quando o usuário entrar no raio... o aplicativo deve identificar a entrada... e disparar a notificação automaticamente", inclusive fechado. O Expo Go não roda tarefa em segundo plano (só build próprio) e a web não tem geofence; o raio mínimo que o sistema aceita monitorar é 100 m (raios menores sobem até esse piso só no monitoramento em segundo plano — com o app aberto vale o raio escolhido) |
| 21/09/2026 | **Busca de endereço reescrita: Photon e Nominatim em paralelo, com CEP pelo ViaCEP e reforço por rua/número no Nominatim** (`src/lib/geocodificar.ts`, `candidatos.ts`, `enderecos.ts`, `geocodificadores.ts`), no lugar de um único pedido de texto livre ao Nominatim; a sugestão cujo número de porta o mapa não tem avisa "Posição aproximada" | Pedido do João: buscar não achava vários endereços de Fortaleza testados por ele, e precisa achar rua, número, bairro, cidade, estado e CEP em qualquer lugar do Brasil. Medido contra os serviços de verdade (21/09/2026, corpus de Fortaleza): o Nominatim sozinho, do jeito que o app usava, devolvia lista vazia com um erro de digitação e às vezes a mesma rua de outra cidade em primeiro quando o texto não dizia a cidade; a Photon tolera erro de digitação e resolve em cerca de um terço do tempo, mas repete a mesma avenida em vários trechos e erra mais o número da porta. O número exato de uma casa só existe no OpenStreetMap em cerca de 1 em cada 4 endereços testados (nenhum dos sorteados na malha residencial); um CEP não é um ponto (a coordenada da BrasilAPI cai no centro da cidade, a alguns quilômetros do endereço) |
| 22/09/2026 | **A abertura perde a foto de fundo:** `bg-onboarding.jpg` (parede verde desfocada, com folhagem e faixas de luz) sai de `src/components/Onboarding.tsx` e o fundo passa a ser o liso `colors.onboarding.bg`. Pino 3D, letreiro manuscrito, textos, balões de vidro e a pastilha "Pular" ficam intactos | Pedido do João: "a única coisa que eu não quero é esse efeito de glass vidrificado, morfismo, que aparece ao redor da imagem, do 'Lembre de tudo'". Não era filtro nenhum — o borrão estava dentro da própria foto. **Divergência deliberada de `docs/referencias/01-onboarding.png`**, que ainda mostra a foto: aqui vale a decisão, não a imagem. `src/components/__tests__/Onboarding.test.tsx` prova a ausência (falha se a camada de imagem voltar; conferido por mutação) |

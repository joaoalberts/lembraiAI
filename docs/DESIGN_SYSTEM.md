...
| 22/09/2026 | **A abertura perdeu a foto de fundo e voltou atrás no mesmo dia.** `bg-onboarding.jpg` saiu de `src/components/Onboarding.tsx` e depois foi reposta; a tela está de novo idêntica ao original (foto, balões de vidro e pager). **Nada mudou no fim** | O João pediu a saída do "efeito de glass vidrificado" — que não era efeito nenhum: o borrão está dentro da própria foto. Com a tela sem foto à vista, ele pediu um fundo com mais profundidade; as alternativas testadas (degradê do design system e fotos abstratas tratadas para a paleta) não o convenceram, e ele preferiu o original de volta. Fica registrado para ninguém repetir o caminho: **a foto é decisão dele, não descuido**. Contexto e o limite de resolução da arte do relógio: `Docs/Decisoes/2026-09-22-abertura-sem-foto-de-fundo.md` |

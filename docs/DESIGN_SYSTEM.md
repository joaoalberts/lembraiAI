...
**O horizonte** (`.horizonte` do `AuthShell` do app web): as mesmas curvas de nível do topo, viradas de cabeça para baixo e esticadas até a altura de 1000 du, com a máscara `linear-gradient(transparent 0 28%, #000 44%, rgba(0,0,0,.5) 58%, transparent 72%)`. O traço só existe entre 28% e 72% dessa altura, então a arte (`assets/art/horizonte-contas.webp`, 1702 × 880, gerada por `scripts/arte/gerar-topo.sh horizonte-contas`) guarda só essa faixa: `size.auth.horizonteAltura` de altura, com a base `size.auth.horizonteBase` acima do fim do conteúdo. Sangra de ponta a ponta (anula o recuo `size.auth.side`), fica dentro do rodapé antes da barra de vidro e da nota, que desenham por cima, e não recebe toque. Acompanha o fim do conteúdo: nas telas altas (cadastro) aparece ao rolar. Conferido no navegador a 430 px, lado a lado com a imagem `02`: as mesmas curvas nos mesmos lugares, com o mesmo esmaecimento sobre a barra de vidro.

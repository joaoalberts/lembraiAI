...
import { DEFAULT_DATE_ISO, DEFAULT_DATE_LABEL } from '../data/reminders';
import { at, box, cx, du } from '../lib/du';
import { formatDate } from '../lib/format';
import { useStore, type Draft } from '../state/store';
import s from './NovoLembrete.module.css';

/** Tela 2 — Novo lembrete (ref/2.png). */
export function NovoLembrete() {
  const nav = useNavigate();
  const { create, update } = useStore();
  // "Editar" (tela de sucesso) abre o formulário já preenchido
  const init = (useLocation().state as { draft?: Draft; editId?: string } | null) ?? {};
  const d0 = init.draft;
  const [title, setTitle] = useState(d0?.title ?? '');
  const [mode, setMode] = useState<'time' | 'place'>(d0?.kind === 'local' && !d0.hasPlace ? 'place' : 'time');
  const [dateISO, setDateISO] = useState(d0?.dateISO ?? DEFAULT_DATE_ISO);
  const [dateLabel, setDateLabel] = useState(d0?.dateLabel ?? DEFAULT_DATE_LABEL);
  ...
  const submit = () => {
    const draft: Draft = { title, kind: mode === 'place' ? 'local' : 'time', dateISO, dateLabel, time, repeat: 'Nunca', place, radius, hasPlace };
    ...
    <Screen>
      ...
      <section className={s.card} style={box(29, 608, 791, 289)}>
        ...
        <SelectField style={box(484, 72, 277, 79)} icon={Clock} value={time} label="Horário do lembrete"
                     inputType="time" inputValue={time} iconX={41} textX={82} chevronX={238} onChange={setTime} />

        <button type="button" className={s.repeat} style={box(30, 171, 730, 95)}>
          ...
          <span className={cx('at', s.repeatValue)} style={at(88, 66)}>Nunca</span>
      ...
      <CtaButton variant="orange" size="form" serif arrow="circle" style={box(29, 1596)} onClick={submit}>Criar lembrete</CtaButton>
      <HomeIndicator variant="form" />
    </Screen>
  );
}
